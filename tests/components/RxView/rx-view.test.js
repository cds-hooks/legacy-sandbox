...
describe('RxView component', () => {
  ...
  it('allows for inputting a number for the dosage amount', () => {
    ...
    expect(updateDosageInstructions).toHaveBeenCalledWith(4, 'daily', '3.0.1', patient.id);
    renderedComponent.find('NumberField').simulate('change', { target: { value: '6' } });
    expect(updateDosageInstructions).toHaveBeenCalledWith(5, 'daily', '3.0.1', patient.id);
    renderedComponent.find('NumberField').simulate('change', { target: { value: '0' } });
    expect(updateDosageInstructions).toHaveBeenCalledWith(1, 'daily', '3.0.1', patient.id);
  });
