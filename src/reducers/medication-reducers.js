...
const initialState = {
   ...
   * keep track of what stage the medication-selection is in (any of the 3 strings above),
...
const filterSearch = (input) => {
  // Break the user input into an array of strings separated by whitespace and filter out any empty strings
...
const createFhirResource = (fhirVersion, patientId, state) => {
  ...
  let startDate;
  let endDate;
  if (state.prescriptionDates.start && state.prescriptionDates.start.enabled) {
    startDate = moment(state.prescriptionDates.start.value).format('YYYY-MM-DD');
  }
  if (state.prescriptionDates.end && state.prescriptionDates.end.enabled) {
  ...
  if (state.selectedCondition) {
    resource[`${isSTU3 ? 'reasonCode' : 'reasonCodeableConcept'}`] = getConditionCodingFromCode(state.selectedConditionCode).resource.code;
  }
  return resource;
};
